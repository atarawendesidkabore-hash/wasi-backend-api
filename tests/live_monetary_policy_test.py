...
Run directly:  python tests/live_monetary_policy_test.py
NOT collected by pytest (all logic gated behind __name__ == "__main__").
"""
import sys
import os
...
if __name__ == "__main__":
    ...
    reg = client.post("/api/auth/register", json={
        "username": "mpolicy_tester",
        "email": "mpolicy@test.local",
    ...
    login_data = login.json() if login.status_code == 200 else {}
    token = login_data.get("access_token", "")
    headers = {"Authorization": f"Bearer {token}"}
    print(f"  Login: {login.status_code} (token={'OK' if token else 'MISSING'})")

    # Promote user to admin (required for topup)
    from src.database.connection import SessionLocal
    from src.database.models import User
    _db = SessionLocal()
    _user = _db.query(User).filter(User.username == "mpolicy_tester").first()
    if _user:
        _user.is_admin = True
        _db.commit()
    _db.close()
